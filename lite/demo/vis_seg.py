...
def img_save_and_viz(
    ...
    # 클래스 정보, 색상 정보, 픽셀 수, 비율 출력
    print("Image:", output_path)
    total_segmented_pixels = np.sum(sem_seg > 0)  # segment된 전체 픽셀 수 계산

    for label, color in zip(labels, colors):
        class_name = classes[label]
        pixel_count = np.sum(sem_seg == label)  # 해당 클래스의 픽셀 수 계산

        if total_segmented_pixels > 0:
            pixel_ratio = (pixel_count / total_segmented_pixels) * 100  # 비율 계산
        else:
            pixel_ratio = 0  # segment된 픽셀이 없는 경우 비율은 0

        print(f"  Class ID: {label}, Class Name: {class_name}, Color: {color}, Pixel Count: {pixel_count}, Ratio: {pixel_ratio:.2f}%")
